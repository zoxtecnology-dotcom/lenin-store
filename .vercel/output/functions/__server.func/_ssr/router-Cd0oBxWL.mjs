import { g as compileDecodeCharMap, j as createLRUCache, a5 as trimPath, a3 as rewriteBasepath, h as composeRewrites, X as processRouteTree, W as processRouteMasks, a2 as resolvePath, e as cleanPath, a7 as trimPathRight, i as createControlledPromise, V as parseHref, s as executeRewriteInput, G as isDangerousProtocol, Z as redirect, K as isRedirect, I as isNotFound, w as findSingleMatch, n as deepEqual, D as DEFAULT_PROTOCOL_ALLOWLIST, d as buildRouteBranch, E as interpolatePath, U as nullReplaceEqualDeep, $ as replaceEqualDeep$1, P as last, m as decodePath, u as findFlatMatch, x as functionalUpdate$1, v as findRouteMatch, J as isPromise, F as invariant, a4 as rootRouteId, a9 as useRouter, N as jsxRuntimeExports, Y as reactExports, O as Outlet, S as notFound, C as hasKeys, t as executeRewriteOutput, p as encodePathLikeUrl, c as arraysEqual, R as React, r as exactPathTest, _ as removeTrailingSlash, H as isModuleNotFoundError, L as isServer$1, o as dummyMatchContext, Q as matchContext, q as escapeHtml, y as getAssetCrossOrigin, A as getScriptPreloadAttrs, b as appendUniqueUserTags, a8 as useHydrated, a6 as trimPathLeft, M as joinPaths, a as React$1, a1 as resolveManifestCssLink, z as getDefaultExportFromCjs, a0 as requireReactDom } from "./index.mjs";
import { c as createClient, _ as __assign, a as __rest, b as __spreadArray } from "./index-0g9BxVXQ.mjs";
...
function decode(str) {
  ...
  for (const [key2, value] of searchParams.entries()) {
    ...
    if (previousValue == null) result[key2] = toValue$1(value);
    else if (Array.isArray(previousValue)) previousValue.push(toValue$1(value));
    else result[key2] = [previousValue, toValue$1(value)];
  }
  return result;
}
...
const appCss = "/assets/styles-DYj17pbs.css";
...
function CartProvider({ children }) {
  ...
  const [freeShippingThreshold, setFreeShippingThreshold] = reactExports.useState(2e5);
  reactExports.useEffect(() => {
    supabase.from("site_settings").select("value").eq("key", "free_shipping_threshold").single().then(({ data }) => {
      if (data?.value) setFreeShippingThreshold(parseInt(data.value) || 2e5);
    });
  }, []);
  function add(item) {
    setItems((prev) => {
  ...
  const count2 = items.reduce((sum, i) => sum + i.qty, 0);
...
const toCamelCase = (string) => string.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (match, p12, p22) => p22 ? p22.toUpperCase() : p12.toLowerCase()
);
...
function SettingsProvider({ children }) {
    ...
    supabase.from("site_settings").select("key, value").then(({ data }) => {
      if (data) {
        const loaded = data.reduce(
          (acc, { key: key2, value }) => ({ ...acc, [key2]: value }),
          {}
        );
  ...
  const freeShippingThreshold = parseInt(settings.free_shipping_threshold) || 2e5;
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
...
function useSettings() {
  ...
  if (!ctx) {
    return {
      ...
      freeShippingThreshold: 2e5
    };
  }
  return ctx;
}
function CartDrawer() {
  const { items, remove, updateQty, total, count: count2, open, setOpen, freeShippingThreshold, amountToFreeShipping } = useCart();
...
function WhatsappWidget() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "fixed bottom-24 right-4 md:bottom-8 md:right-6 z-40 flex flex-col items-end gap-3", children: [
    open && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-[calc(100vw-2rem)] max-w-[22rem] bg-background border border-border overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-bottom-right", children: [
...
function WishlistProvider({ children }) {
  ...
  reactExports.useEffect(() => {
    async function load() {
      ...
      if (user) {
        ...
        const dbItems = data?.map((w) => w.product_id).filter(Boolean) ?? [];
        setItems(dbItems);
        try {
          const localItems = JSON.parse(localStorage.getItem("aiahn-wishlist") ?? "[]");
  ...
  const toggle = reactExports.useCallback(async (productId) => {
  ...
  const has = reactExports.useCallback((productId) => items.includes(productId), [items]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(WishlistContext.Provider, { value: { items, toggle, has, loading }, children });
}
...
function RootComponent() {
  ...
  reactExports.useEffect(() => {
    function measureHeader() {
      const el2 = document.getElementById("site-header");
      if (el2) {
        document.documentElement.style.setProperty("--header-h", `${el2.offsetHeight}px`);
      }
    }
    measureHeader();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(QueryClientProvider, { client: queryClient, children: /* @__PURE__ */ jsxRuntimeExports.jsx(SettingsProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(AuthProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(WishlistProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(CartProvider, { children: [
...
async function fetchProducts() {
  const { data, error } = await supabase.from("visible_products").select(PRODUCT_SELECT).order("position");
  if (error) throw error;
  return data.map(rowToProduct);
}
async function fetchProductBySlug(slug) {
  const { data, error } = await supabase.from("visible_products").select(PRODUCT_SELECT).eq("slug", slug).maybeSingle();
  if (error) throw error;
  return data ? rowToProduct(data) : null;
}
async function fetchProductsNewest(limit) {
  let query = supabase.from("visible_products").select(PRODUCT_SELECT).order("created_at", { ascending: false });
...
async function fetchProductsIsNew() {
  const { data, error } = await supabase.from("visible_products").select(PRODUCT_SELECT).eq("is_new", true).order("created_at", { ascending: false });
  if (error) throw error;
  return data.map(rowToProduct);
}
async function fetchMostSoldProducts() {
  ...
  const { data, error } = await supabase.from("visible_products").select(PRODUCT_SELECT).in("id", sorted);
  if (error) throw error;
  const products2 = data.map(rowToProduct);
  return sorted.map((id) => products2.find((p) => data.find((r2) => r2.slug === p.slug && r2.id === id))).filter((p) => !!p);
}
...
function rowToDrop(row) {
  return {
    ...
    editorialCaptions: [...row.drop_images].sort((a, b) => a.position - b.position).map((i) => i.alt_text ?? ""),
...
async function fetchUpcomingDrop() {
  const { data, error } = await supabase.from("drops").select(DROP_SELECT).eq("published", false).order("position").limit(1).maybeSingle();
...
async function fetchProductsByDrop(dropId) {
  const { data, error } = await supabase.from("visible_products").select(PRODUCT_SELECT).eq("drop_id", dropId).order("position");
  if (error) throw error;
  return data.map(rowToProduct);
}
const catalog = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  fetchCollection,
  fetchCollections,
  fetchDropBySlug,
...
const $$splitComponentImporter$I = () => import("./wishlist-Bhx38ggg.mjs");
const Route$I = createFileRoute("/wishlist")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$H = () => import("./tiendas-BZU73HbN.mjs");
const Route$H = createFileRoute("/tiendas")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$G = () => import("./terminos-DXCC2HDr.mjs");
const Route$G = createFileRoute("/terminos")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$F = () => import("./privacidad-CHfOfAie.mjs");
const Route$F = createFileRoute("/privacidad")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$E = () => import("./prensa-CIlrpA_P.mjs");
const Route$E = createFileRoute("/prensa")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$D = () => import("./packs-GPQ95rbD.mjs");
const Route$D = createFileRoute("/packs")({
  component: lazyRouteComponent($$splitComponentImporter$D, "component")
});
const $$splitComponentImporter$C = () => import("./login-Dwfad44Z.mjs");
...
const $$splitComponentImporter$B = () => import("./historia-CNVbgh9m.mjs");
const Route$B = createFileRoute("/historia")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$A = () => import("./faq-CJCvqBc1.mjs");
...
const $$splitComponentImporter$z = () => import("./envios-D-c12bUr.mjs");
const Route$z = createFileRoute("/envios")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$y = () => import("./drops-GPQ95rbD.mjs");
const Route$y = createFileRoute("/drops")({
  component: lazyRouteComponent($$splitComponentImporter$y, "component")
});
const $$splitComponentImporter$x = () => import("./cuenta-w82fz2tn.mjs");
const Route$x = createFileRoute("/cuenta")({
  component: lazyRouteComponent($$splitComponentImporter$x, "component")
});
const $$splitComponentImporter$w = () => import("./cookies-Bycojr7l.mjs");
const Route$w = createFileRoute("/cookies")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$v = () => import("./contacto-BCOscJww.mjs");
const Route$v = createFileRoute("/contacto")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$u = () => import("./cambios-devoluciones-BUB-dai8.mjs");
const Route$u = createFileRoute("/cambios-devoluciones")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$t = () => import("./aviso-Cqj4vnWM.mjs");
const Route$t = createFileRoute("/aviso")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$s = () => import("./admin-B4SmokBS.mjs");
const Route$s = createFileRoute("/admin")({
  component: lazyRouteComponent($$splitComponentImporter$s, "component")
});
const $$splitComponentImporter$r = () => import("./index-DJdXGBDa.mjs");
...
const $$splitComponentImporter$q = () => import("./index-XkP4qGXK.mjs");
const Route$q = createFileRoute("/packs/")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$p = () => import("./index-DNx9JoXS.mjs");
const Route$p = createFileRoute("/drops/")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$o = () => import("./index-DilurmsY.mjs");
const Route$o = createFileRoute("/cuenta/")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$n = () => import("./index-DQ_PGevc.mjs");
const Route$n = createFileRoute("/admin/")({
  component: lazyRouteComponent($$splitComponentImporter$n, "component")
});
const $$splitComponentImporter$m = () => import("../_slug-BAQZcjHO.mjs");
const Route$m = createFileRoute("/products/$slug")({
  head: ({
    loaderData
  }) => ({
  ...
  loader: async ({
    ...
    const [product, allProducts, settingsRes] = await Promise.all([fetchProductBySlug(params.slug), fetchProducts(), supabase.from("site_settings").select("key, value").in("key", ["free_shipping_threshold", "returns_days", "shipping_time"])]);
...
const $$splitComponentImporter$l = () => import("../_id-DwtrIfHK.mjs");
const Route$l = createFileRoute("/packs/$id")({
  head: ({
    loaderData
  }) => ({
    meta: [{
  ...
  loader: async ({
    ...
    const {
      fetchPacks: fetchPacks2
    } = await Promise.resolve().then(() => catalog);
    const [pack, allPacks] = await Promise.all([fetchPackBySlug(params.id), fetchPacks2()]);
...
const $$splitComponentImporter$k = () => import("../_id-Dyi7oHJk.mjs");
const Route$k = createFileRoute("/drops/$id")({
  head: ({
    loaderData
  }) => ({
    meta: [{
...
const $$splitComponentImporter$j = () => import("./perfil-D4ZvzDLQ.mjs");
const Route$j = createFileRoute("/cuenta/perfil")({
  head: () => ({
    meta: [{
      title: pageTitle("Editar perfil")
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$j, "component")
});
const $$splitComponentImporter$i = () => import("./pedidos-ByQdyqU6.mjs");
const Route$i = createFileRoute("/cuenta/pedidos")({
  head: () => ({
    meta: [{
      title: pageTitle("Mis pedidos")
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$i, "component")
});
const $$splitComponentImporter$h = () => import("./direcciones-Cgx49DK8.mjs");
const Route$h = createFileRoute("/cuenta/direcciones")({
  head: () => ({
    meta: [{
      title: pageTitle("Mis direcciones")
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$h, "component")
});
...
ZodOptional.create;
ZodNullable.create;
const $$splitComponentImporter$g = () => import("../_handle-Ca9f5jnA.mjs");
...
const Route$g = createFileRoute("/collections/$handle")({
...
const $$splitComponentImporter$f = () => import("./callback-qTMxE87h.mjs");
const Route$f = createFileRoute("/auth/callback")({
  component: lazyRouteComponent($$splitComponentImporter$f, "component")
});
const $$splitComponentImporter$e = () => import("./tallas-qoc-ekVN.mjs");
const Route$e = createFileRoute("/admin/tallas")({
  component: lazyRouteComponent($$splitComponentImporter$e, "component")
});
const $$splitComponentImporter$d = () => import("./configuracion-C667CpiI.mjs");
const Route$d = createFileRoute("/admin/configuracion")({
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const $$splitComponentImporter$c = () => import("./colores-CPG4HUCJ.mjs");
const Route$c = createFileRoute("/admin/colores")({
  component: lazyRouteComponent($$splitComponentImporter$c, "component")
});
const $$splitComponentImporter$b = () => import("./clientes-BHp8JRmw.mjs");
const Route$b = createFileRoute("/admin/clientes")({
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("./index-C0jkq4RZ.mjs");
const Route$a = createFileRoute("/admin/productos/")({
  component: lazyRouteComponent($$splitComponentImporter$a, "component")
});
const $$splitComponentImporter$9 = () => import("./index-B_rq0-Gj.mjs");
const Route$9 = createFileRoute("/admin/pedidos/")({
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("./index-CxriH9Zz.mjs");
const Route$8 = createFileRoute("/admin/packs/")({
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("./index-Dpl1v_f0.mjs");
const Route$7 = createFileRoute("/admin/drops/")({
  component: lazyRouteComponent($$splitComponentImporter$7, "component")
});
const $$splitComponentImporter$6 = () => import("./nuevo-DH3nDsi7.mjs");
const Route$6 = createFileRoute("/admin/productos/nuevo")({
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("../_id-CCZWJpn6.mjs");
const Route$5 = createFileRoute("/admin/productos/$id")({
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("../_id-D_x1aSmp.mjs");
const Route$4 = createFileRoute("/admin/pedidos/$id")({
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./nuevo-DlCOtqYR.mjs");
const Route$3 = createFileRoute("/admin/packs/nuevo")({
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("../_id-BlDWVJft.mjs");
const Route$2 = createFileRoute("/admin/packs/$id")({
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("./nuevo-BiokbmJp.mjs");
const Route$1 = createFileRoute("/admin/drops/nuevo")({
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("../_id-C0bURxSJ.mjs");
const Route2 = createFileRoute("/admin/drops/$id")({
  component: lazyRouteComponent($$splitComponentImporter, "component")
});
const WishlistRoute = Route$I.update({
  id: "/wishlist",
  path: "/wishlist",
  getParentRoute: () => Route$J
});
const TiendasRoute = Route$H.update({
  id: "/tiendas",
  path: "/tiendas",
  getParentRoute: () => Route$J
});
const TerminosRoute = Route$G.update({
  id: "/terminos",
  path: "/terminos",
  getParentRoute: () => Route$J
});
const PrivacidadRoute = Route$F.update({
  id: "/privacidad",
  path: "/privacidad",
  getParentRoute: () => Route$J
});
const PrensaRoute = Route$E.update({
  id: "/prensa",
  path: "/prensa",
  getParentRoute: () => Route$J
});
const PacksRoute = Route$D.update({
  id: "/packs",
  path: "/packs",
  getParentRoute: () => Route$J
});
...
const HistoriaRoute = Route$B.update({
  id: "/historia",
  path: "/historia",
  getParentRoute: () => Route$J
});
...
const EnviosRoute = Route$z.update({
  id: "/envios",
  path: "/envios",
  getParentRoute: () => Route$J
});
const DropsRoute = Route$y.update({
  id: "/drops",
  path: "/drops",
  getParentRoute: () => Route$J
});
const CuentaRoute = Route$x.update({
  id: "/cuenta",
  path: "/cuenta",
  getParentRoute: () => Route$J
});
const CookiesRoute = Route$w.update({
  id: "/cookies",
  path: "/cookies",
  getParentRoute: () => Route$J
});
const ContactoRoute = Route$v.update({
  id: "/contacto",
  path: "/contacto",
  getParentRoute: () => Route$J
});
const CambiosDevolucionesRoute = Route$u.update({
  id: "/cambios-devoluciones",
  path: "/cambios-devoluciones",
  getParentRoute: () => Route$J
});
const AvisoRoute = Route$t.update({
  id: "/aviso",
  path: "/aviso",
  getParentRoute: () => Route$J
});
...
const PacksIndexRoute = Route$q.update({
  id: "/",
  path: "/",
  getParentRoute: () => PacksRoute
});
const DropsIndexRoute = Route$p.update({
  id: "/",
  path: "/",
  getParentRoute: () => DropsRoute
});
const CuentaIndexRoute = Route$o.update({
  id: "/",
  path: "/",
...
const ProductsSlugRoute = Route$m.update({
  id: "/products/$slug",
  path: "/products/$slug",
  getParentRoute: () => Route$J
});
const PacksIdRoute = Route$l.update({
  id: "/$id",
  path: "/$id",
  getParentRoute: () => PacksRoute
});
const DropsIdRoute = Route$k.update({
  id: "/$id",
  path: "/$id",
  getParentRoute: () => DropsRoute
});
const CuentaPerfilRoute = Route$j.update({
  id: "/perfil",
  path: "/perfil",
  getParentRoute: () => CuentaRoute
});
const CuentaPedidosRoute = Route$i.update({
  id: "/pedidos",
  path: "/pedidos",
  getParentRoute: () => CuentaRoute
});
const CuentaDireccionesRoute = Route$h.update({
  id: "/direcciones",
  path: "/direcciones",
  getParentRoute: () => CuentaRoute
});
const CollectionsHandleRoute = Route$g.update({
  id: "/collections/$handle",
  path: "/collections/$handle",
  getParentRoute: () => Route$J
});
const AuthCallbackRoute = Route$f.update({
  id: "/auth/callback",
  path: "/auth/callback",
  getParentRoute: () => Route$J
});
const AdminTallasRoute = Route$e.update({
  id: "/tallas",
  path: "/tallas",
  getParentRoute: () => AdminRoute
});
const AdminConfiguracionRoute = Route$d.update({
  id: "/configuracion",
  path: "/configuracion",
  getParentRoute: () => AdminRoute
});
const AdminColoresRoute = Route$c.update({
  id: "/colores",
  path: "/colores",
  getParentRoute: () => AdminRoute
});
...
const AdminDropsIdRoute = Route2.update({
  id: "/drops/$id",
  path: "/drops/$id",
  getParentRoute: () => AdminRoute
});
...
const CuentaRouteChildren = {
  CuentaDireccionesRoute,
  CuentaPedidosRoute,
  CuentaPerfilRoute,
  CuentaIndexRoute
};
const CuentaRouteWithChildren = CuentaRoute._addFileChildren(CuentaRouteChildren);
